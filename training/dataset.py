...
class VideoDataset(Dataset):
    ...
    def _load_dataset_from_local_path(self) -> Tuple[List[str], List[str]]:
        ...
        if self.video_column.endswith('.json') and self.video_column.endswith('.json'):
            meta_info = json.load(open(self.video_column))
            video_paths = [entry['path'] for entry in meta_info if 'path' in entry]
            prompts = [entry['text'] for entry in meta_info if 'text' in entry]
            # start_frame = [entry['start'] for entry in meta_info if 'start' in entry]
            # end_frame = [entry['end'] for entry in meta_info if 'end' in entry]
        else:
...
class VideoDatasetWithResizing(VideoDataset):
    ...
    def _preprocess_video(self, path: Path) -> torch.Tensor:
        if self.load_tensors:
            ...
            video_num_frames = len(video_reader)
            current_fps = video_reader.get_avg_fps() 
            target_fps = 8
            frame_skip = int(current_fps // target_fps) 
            ...
            frame_indices = list(range(0, video_num_frames, frame_skip))
            while len(frame_indices) < nearest_frame_bucket  and nearest_frame_bucket-len(frame_indices)>1 and frame_skip > 1:
